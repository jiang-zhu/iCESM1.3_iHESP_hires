...
subroutine diag_init()
  ...
  ! Declare the history fields for which this module contains outfld calls.
   ...
   call addfld (bpcnst(1) ,'kg/kg   ',pver, 'A',trim(cnst_longname(1))//' (before physics)',phys_decomp)
   ! State after physics
   ...
   call addfld (apcnst(1) ,'kg/kg   ',pver, 'A',trim(cnst_longname(1))//' (after physics)',phys_decomp)
   ...
   ! isotopes. -JN
   if(trace_water) then !Are water tracers on? 
     call addfld ('QFLX_H2O','kg/m2/s ',1,    'A','Surface water H2O tracer flux',phys_decomp)
     call addfld ('QFLX_16O','kg/m2/s ',1,    'A','Surface water H216O tracer flux',phys_decomp)
     call addfld ('QFLX_HDO','kg/m2/s ',1,    'A','Surface water HD16O tracer flux',phys_decomp)
     call addfld ('QFLX_18O','kg/m2/s ',1,    'A','Surface water H218O tracer flux',phys_decomp)
...
!
   ...
   ! isotopes. -JN
   if(trace_water) then
     call outfld('QFLX_H2O', cam_in%cflx(1,wtrc_iatype(1,iwtvap)), pcols, lchnk)
     call outfld('QFLX_16O', cam_in%cflx(1,wtrc_iatype(2,iwtvap)), pcols, lchnk)
     call outfld('QFLX_HDO', cam_in%cflx(1,wtrc_iatype(3,iwtvap)), pcols, lchnk)
     call outfld('QFLX_18O', cam_in%cflx(1,wtrc_iatype(4,iwtvap)), pcols, lchnk)
