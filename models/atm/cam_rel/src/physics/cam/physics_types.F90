...
!---------------------------Local storage-------------------------------
    ...
    !water tracers:
    !-------------
    integer :: ixh2oi, wtfin   !water tracer/isotope indices
    !-------------
   ...
   !itself passes. -JN 
    ...
    call cnst_get_ind('H2OI', ixh2oi, abort=.false.)    !Look for water tracer liquid

    if(ixh2oi .ne. -1) then                             !Water tracers are present

      !---------------------------------------------
      !Calculate how many water isotopes are present:
      !---------------------------------------------
      wtfin = ixh2oi    !initalize loop limit to be H2O

      do m=ixh2oi,pcnst !loop over constituents
        if(scan(trim(cnst_name(m)),'I',.true.) .eq. len(trim(cnst_name(m)))) then  !Does the consitutent name end in "I"?
          wtfin = m                                                                !set loop limit
        end if
      end do
      ...
      !Cloud liquid:
      !-------------
      do m=ixh2oi-1,wtfin-1,7 !only loop over liquid condensate tracers
        if(ptend%lq(m)) then
#ifdef PERGRO
          if ( any(ptend%name == pergro_cldlim_names) ) &
            call state_cnst_min_nz(1.e-12_r8, ixcldliq, m)
#endif
          if (ptend%name == 'convect_deep') &
            call state_cnst_min_nz(1.e-36_r8, ixcldliq, m)
        end if
      end do
      !---------
      !Cloud Ice:
      !---------
      do m=ixh2oi,wtfin,7  !only loop over ice condensate tracers
        if(ptend%lq(m)) then
#ifdef PERGRO
          if ( any(ptend%name == pergro_cldlim_names) ) &
            call state_cnst_min_nz(1.e-12_r8, ixcldice, m)
#endif
          if (ptend%name == 'convect_deep') &
            call state_cnst_min_nz(1.e-36_r8, ixcldice, m)
        end if
      end do
      !---------
    end if
