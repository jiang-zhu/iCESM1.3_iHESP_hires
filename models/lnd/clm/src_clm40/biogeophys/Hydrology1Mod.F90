    ...
    do f = 1, num_nolakep
       ...
       if (ctype(c) /= icol_sunwall .and. ctype(c) /= icol_shadewall) then
          if (frac_veg_nosno(p) == 0) then
             ...
             do m = 1, pwtrc
               wtr_qflx_prec_grnd_snow(p,m) = wtr_qflx_through_snow(p,m) + &
                          (wtr_qflx_candrip(p,m) * fracsnow(p))
               wtr_qflx_prec_grnd_rain(p,m) = wtr_qflx_through_rain(p,m) + &
                          (wtr_qflx_candrip(p,m) * fracrain(p)) + wtr_h2ocan_loss(c,m)
             end do
          end if
