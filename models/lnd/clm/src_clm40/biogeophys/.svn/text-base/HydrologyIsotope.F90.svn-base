...
!       rnat  = (/ 1.    ,155.76e-6,2005.20e-6  ,402.00e-6 , 77.88e-06/)   ! natural abundance
       rnat  = (/ 1._r8    ,1.0_r8    ,1.0_r8    ,1.0_r8    ,1.0_r8     /)  ! equal numerics
!       rnat  = (/ 1._r8    ,0.7_r8    ,0.5_r8    ,1.0_r8    ,1.0_r8     /)  ! constant fraction test
...
!        wiso_alpliq = exp(alpal_real(isp)/(tk**2) + alpbl_real(isp)/tk + alpcl_real(isp))
        !Horita and Wesolowski, 1994:
        if(isp == isphdo) then !HDO has different formulation:
          wiso_alpliq = exp(alpal_real(isp)*tk**3 + alpbl_real(isp)*tk**2 + &
                        alpcl_real(isp)*tk + alpdl_real(isp) + alpel_real(isp)/tk**3)
        else
          wiso_alpliq = exp(alpal_real(isp)/tk**3 + alpbl_real(isp)/tk**2 + &
                        alpcl_real(isp)/tk + alpdl_real(isp))
        end if
        wiso_alpliq = wiso_alpliq**expa(isp)
#ifdef NOFRAC
        wiso_alpliq = 1._r8
