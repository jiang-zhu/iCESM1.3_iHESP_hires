...
!----------------------------------------------------
  ...
  type, public :: atm2lnd_downscaled_fields_type
     ...
     real(r8), pointer :: forc_wtr_q(:,:) => null() !tracer atmospheric specific humidity (kg/kg)
     ...
     real(r8), pointer :: forc_wtr_rain(:,:) => null() !tracer rain rate [mm/s]
     real(r8), pointer :: forc_wtr_snow(:,:) => null() !tracer snow rate [mm/s]
     ...
     real(r8), pointer :: forc_wtr_flood(:,:) => null() ! tracer rof flood (mm/s)
     real(r8), pointer :: forc_wtr_Rclim(:,:) => null() ! tracer ratio climatology (deep soil)
...
!----------------------------------------------------
  ...
  type, public :: lnd2atm_type
     ...
     real(r8), pointer :: wtr_q_ref2m(:,:) => null() !tracer 2m surface specific humidity (kg/kg)
     real(r8), pointer :: u_ref10m(:)     => null() !10m surface wind speed (m/sec)
     real(r8), pointer :: h2osno(:)       => null() !snow water (mm H2O)
     real(r8), pointer :: wtr_h2osno(:,:) => null() !tracer snow water (mm H2O)
     ...
     real(r8), pointer :: wtr_qflx_evap_tot(:,:) => null() !tracer qflx_evap_soi + qflx_evap_can + qflx_tran_veg
     ...
     real(r8), pointer :: wtr_rofliq(:,:) => null() ! tracer rof liq forcing
     real(r8), pointer :: wtr_rofice(:,:) => null() ! tracer rof ice forcing
...
!------------------------------------------------------------------------
  ...
  a2l%forc_wtr_rain(beg:end,:) = ival
  a2l%forc_wtr_snow(beg:end,:) = ival
  a2l%forc_ndep(beg:end) = ival
  a2l%forc_pc13o2(beg:end) = ival
  a2l%forc_po2(beg:end) = ival
  a2l%forc_flood(beg:end) = ival
  a2l%forc_wtr_flood(beg:end,:) = ival
  a2l%forc_wtr_Rclim(beg:end,:) = ival
