...
!-----------------------------------------------------------------------
    ...
    call mpi_bcast (hydro_tracer,     1, MPI_LOGICAL,     0, mpicom, ier)
    call mpi_bcast (tracer_forc,      1, MPI_LOGICAL,     0, mpicom, ier)
...
!------------------------------------------------------------------------
    ...
    if ( hydro_tracer ) then
       write(iulog, *)    '   water tracers are ON'
       if ( tracer_forc ) then
          write(iulog, *) '   use isotopic forcing from atmospheric model'
       end if
    end if
    if (fsnowoptics == ' ') then
