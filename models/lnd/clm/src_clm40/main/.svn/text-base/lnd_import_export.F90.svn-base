...
contains
  ...
  subroutine lnd_import( bounds, x2l, a2l, a2l_not_downscaled_gcell, x2s, atm_prognostic)
    ...
    use HydrologyTracer , only: Rstnd, pwtrc, hydro_tracer, tracer_forc, get_wratio, qflxtiny
    implicit none
    !
    ! !ARGUMENTS:
    ...
    logical                              , intent(in)    :: atm_prognostic         ! coupled to CAM?
    !
    ! !LOCAL VARIABLES:
    integer  :: g,i,m,nstep,ier      ! indices, number of steps, and error code
    ...
    do g = bounds%begg,bounds%endg
       ...
       ! Get the tracer forcing
       if(hydro_tracer) then
          if(.not.atm_prognostic .and. tracer_forc) then      ! no iCAM, but have tracer forcing
             a2l%forc_wtr_flood(g,1) = -x2l(index_x2l_Flrr_flood_16O,i)
             a2l%forc_wtr_flood(g,2) = -x2l(index_x2l_Flrr_flood_HDO,i)
             a2l%forc_wtr_flood(g,3) = -x2l(index_x2l_Flrr_flood_18O,i)

             a2l_not_downscaled_gcell%forc_wtr_q(g,1) = x2l(index_x2l_Sa_shum_16O,i)
             a2l_not_downscaled_gcell%forc_wtr_q(g,2) = x2l(index_x2l_Sa_shum_HDO,i)
             a2l_not_downscaled_gcell%forc_wtr_q(g,3) = x2l(index_x2l_Sa_shum_18O,i)

             wtr_forc_rainc(1) = x2l(index_x2l_Faxa_rainc_16O,i)   ! mm/s
             wtr_forc_rainl(1) = x2l(index_x2l_Faxa_rainl_16O,i)   ! mm/s
             wtr_forc_snowc(1) = x2l(index_x2l_Faxa_snowc_16O,i)   ! mm/s
             wtr_forc_snowl(1) = x2l(index_x2l_Faxa_snowl_16O,i)   ! mm/s
             wtr_forc_rainc(2) = x2l(index_x2l_Faxa_rainc_HDO,i)   ! mm/s
             wtr_forc_rainl(2) = x2l(index_x2l_Faxa_rainl_HDO,i)   ! mm/s
             wtr_forc_snowc(2) = x2l(index_x2l_Faxa_snowc_HDO,i)   ! mm/s
             wtr_forc_snowl(2) = x2l(index_x2l_Faxa_snowl_HDO,i)   ! mm/s
             wtr_forc_rainc(3) = x2l(index_x2l_Faxa_rainc_18O,i)   ! mm/s
             wtr_forc_rainl(3) = x2l(index_x2l_Faxa_rainl_18O,i)   ! mm/s
             wtr_forc_snowc(3) = x2l(index_x2l_Faxa_snowc_18O,i)   ! mm/s
             wtr_forc_snowl(3) = x2l(index_x2l_Faxa_snowl_18O,i)   ! mm/s
             do m =1, pwtrc
                a2l%forc_wtr_flood(g,m) = -x2l(index_x2l_Flrr_flood,i)*Rstnd(m)
                a2l%forc_wtr_Rclim(g,m) = Rstnd(m)
                ! Convert from delta-value forcing (per mille) to flux (mm/s)
                wtr_forc_rainc(m) = forc_rainc*Rstnd(m)*((wtr_forc_rainc(m)/1000._r8)+1._r8)
                wtr_forc_rainl(m) = forc_rainl*Rstnd(m)*((wtr_forc_rainl(m)/1000._r8)+1._r8)
                wtr_forc_snowc(m) = forc_snowc*Rstnd(m)*((wtr_forc_snowc(m)/1000._r8)+1._r8)
                wtr_forc_snowl(m) = forc_snowl*Rstnd(m)*((wtr_forc_snowl(m)/1000._r8)+1._r8)
                ! Finally, put them together
                a2l_not_downscaled_gcell%forc_wtr_rain(g,m) = wtr_forc_rainc(m) + wtr_forc_rainl(m)
                a2l_not_downscaled_gcell%forc_wtr_snow(g,m) = wtr_forc_snowc(m) + wtr_forc_snowl(m)
             end do
          elseif(.not.atm_prognostic .and. .not.tracer_forc) then ! no iCAM, but need to make up forcing
             do m=1, pwtrc
                a2l%forc_wtr_flood(g,m) = -x2l(index_x2l_Flrr_flood,i)*Rstnd(m)
                a2l%forc_wtr_Rclim(g,m) = Rstnd(m)
                a2l_not_downscaled_gcell%forc_wtr_q(g,m) = x2l(index_x2l_Sa_shum,i)*Rstnd(m)
                a2l_not_downscaled_gcell%forc_wtr_rain(g,m) = a2l_not_downscaled_gcell%forc_rain(g)*Rstnd(m)
                a2l_not_downscaled_gcell%forc_wtr_snow(g,m) = a2l_not_downscaled_gcell%forc_snow(g)*Rstnd(m)
             end do
          elseif(atm_prognostic) then  ! coupled to iCAM
             a2l%forc_wtr_flood(g,1) = -x2l(index_x2l_Flrr_flood_16O,i)
             a2l%forc_wtr_flood(g,2) = -x2l(index_x2l_Flrr_flood_HDO,i)
             a2l%forc_wtr_flood(g,3) = -x2l(index_x2l_Flrr_flood_18O,i)
  ...
  subroutine lnd_export( bounds, clm_l2a, clm_s2x, l2x, atm_prognostic)
    ...
    use HydrologyIsotope   , only : isph2o, isphdo, isph218o
    use HydrologyTracer    , only : hydro_tracer
    !
    ! !ARGUMENTS:
    implicit none
    type(bounds_type) , intent(in)    :: bounds  ! bounds
    ...
    do g = bounds%begg,bounds%endg
       ...
       if(hydro_tracer) then
          l2x(index_l2x_Sl_snowh_16O,i)  =  clm_l2a%wtr_h2osno(g,isph2o)
          l2x(index_l2x_Sl_snowh_HDO,i)  =  clm_l2a%wtr_h2osno(g,isphdo)
          l2x(index_l2x_Sl_snowh_18O,i)  =  clm_l2a%wtr_h2osno(g,isph218o)

          l2x(index_l2x_Sl_qref_16O,i)   =  clm_l2a%wtr_q_ref2m(g,isph2o)
          l2x(index_l2x_Sl_qref_HDO,i)   =  clm_l2a%wtr_q_ref2m(g,isphdo)
          l2x(index_l2x_Sl_qref_18O,i)   =  clm_l2a%wtr_q_ref2m(g,isph218o)

          l2x(index_l2x_Fall_evap_16O,i) = -clm_l2a%wtr_qflx_evap_tot(g,isph2o)
          l2x(index_l2x_Fall_evap_HDO,i) = -clm_l2a%wtr_qflx_evap_tot(g,isphdo)
          l2x(index_l2x_Fall_evap_18O,i) = -clm_l2a%wtr_qflx_evap_tot(g,isph218o)
       end if
       ...
       if(hydro_tracer) then
          l2x(index_l2x_Flrl_rofl_16O,i) = clm_l2a%wtr_rofliq(g,isph2o)
          l2x(index_l2x_Flrl_rofi_16O,i) = clm_l2a%wtr_rofice(g,isph2o)
          l2x(index_l2x_Flrl_rofl_HDO,i) = clm_l2a%wtr_rofliq(g,isphdo)
          l2x(index_l2x_Flrl_rofi_HDO,i) = clm_l2a%wtr_rofice(g,isphdo)
          l2x(index_l2x_Flrl_rofl_18O,i) = clm_l2a%wtr_rofliq(g,isph218o)
          l2x(index_l2x_Flrl_rofi_18O,i) = clm_l2a%wtr_rofice(g,isph218o)
       end if
       ! glc coupling
